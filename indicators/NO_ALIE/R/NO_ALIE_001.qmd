...
::: {.callout-note}
This indicator was developed in 2023. In 2024 we further developed the ideas 
about HIAs and stratified aggregation which we published on [EcoEvoRxiv](https://ecoevorxiv.org/repository/view/7221/).
Before operationalisation, the indicator should be updated based on this paper.
:::
...
We then calculate an area weighted mean (and error) indicator value for each HIA, 
as long as there is more than *n* data points for a given combination of HIA and municipality.
...
The variables are recorded on a unitless seven-step ordinal scale (Fig. \@ref(fig:R7)) or a as a percentage cover of alien species.
...
Now I will convert these percentage cover values into 7FA classes. This is a critical step, since there is no one-to-one mapping here, but rather there is considerable overlap, as can be seen in Fig. \@ref(fig:fremmedart-prosentVSinnslag).
...
```{r alien-precentageplot, fig.cap="Relative frequency plot showing the distribution of polygons with different indicator values."}
